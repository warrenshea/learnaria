;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype.init = function () {
		...
		$elem
			.attr({
				...
				'aria-describedby': id + '_instructions' // associate with instructions (Criteria 1)
			})
			.addClass('ik_carousel')
			.on('keydown', {'plugin': plugin}, plugin.onKeyDown)
			.on('focusin mouseenter', {'plugin': plugin}, plugin.stopTimer) //Criteria 5: While on focus, the timer is stopped (only manual interaction can be done)
			.on('focusout mouseleave', {'plugin': plugin}, plugin.startTimer) //Criteria 6: While off focus, the timer is started (automating rotating)
		...
		$('<div/>') // add instructions for screen reader users (Criteria 1)
			.attr({
				'id': id + '_instructions',
				'aria-hidden': 'true'
			})
		...
		plugin.slides.first().addClass('active'); //Criteria 2 via .active class having display: flex
	...
	Plugin.prototype.startTimer = function (event) {
		...
		if (event.type === 'focusout') { //Criteria 4: Panels read aloud via aria-live: polite
	...
	*/
	Plugin.prototype.onKeyDown = function (event) {
		...
		switch (event.keyCode) { //Criteria 3
